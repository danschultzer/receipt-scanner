...
function handleMatrix(im, outfile, callback, config) {
  try {
    ...
    if (config.removeNoise) im = removeNoise(im, outfile, config);
  ...
  if (mask.channels() == 3) {
    ...
    if (config.verbose) {
      ...
      for(var c = 0; c < contours.size(); ++c) {
        im.drawContour(contours, c, [0, 255, 0], 5);
      }
      im.save(filename);
...
function findPaperInPhoto(im, outfile, config) {
    ...
    if (paperContoursList.length >= 1) {
      ...
      if (config.verbose) {
        var filename = debugFilename(outfile);
        im = im.copy();
        if (im.channels() == 1) im.cvtColor('CV_GRAY2BGR');
...
function getTextElementsContours(im) {
  mask = im.copy();
  if (mask.channels() > 1) mask.convertGrayscale();
  mask.canny(50, 150);
  structure = cv.imgproc.getStructuringElement(1, [6, 6]);
...
function rotate(paperMask, im, outfile, config) {
  ...
  contoursList = contoursList.sort(function(a, b) { return a[1].angle - b[1].angle; });
  ...
  maxDif = 1.0;
  if (Math.abs(median - contoursList[contoursList.length - 1][1].angle) > maxDif || Math.abs(contoursList[0][1].angle - median) > maxDif) return im;
...
function removeNoise(im, outfile, config) {
    if (im.channels() == 3) im.convertGrayscale();
