...
function findPaperInPhoto(im, outfile, config) {
    // Find paper
    paperMask = getPaperThreshold(im, outfile, config);
...
function rotate(paperMask, im, outfile, config) {
  ...
  // This is not necessary if the paper haven't been deteced
  if(paperMask && config.onlyFindTextInsidePaperContour)
    contoursList = onlyFindTextInsidePaperContour(contours, contoursList, paperMask, config);
...
function onlyFindTextInsidePaperContour(contours, contoursList, paperMask, config) {
  ...
  if (config.verbose) {
    ...
    config.log('verbose[onlyFindTextInsidePaperContour]: Removed ' + removedContoursCount + ' contours.');
  }
